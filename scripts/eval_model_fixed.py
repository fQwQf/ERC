#!/usr/bin/env python
"""
Fixed Evaluation Script for Fine-Grained Emotion Recognition

This script matches the training prompt format exactly.
"""
...
def evaluate(model, tokenizer, samples, max_length=512, batch_size=4):
    ...
    # IMPORTANT: Match training configuration exactly
    # train_multi_gpu.py used: EmotionPromptBuilder(use_retrieval=False)
    ...
    for sample in tqdm(samples, desc="Evaluating"):
        # FIXED: Build prompt with prev_impact to match training format
        # This matches the training format in train_multi_gpu.py
        prev_impact = None
        if sample.prev_emotion:
            prev_impact = f"Previous emotion was {sample.prev_emotion}."
        
        # Use build_training_prompt format but without the target response
        # Actually, we need to build inference prompt with the same structure
        prompt = builder.build_inference_prompt(
            dialogue_history=sample.dialogue_history,
            target_utterance=sample.target_utterance,
            # Note: build_inference_prompt doesn't accept prev_impact directly
            # We need to modify the approach
        )
        
        # Alternative: Build training prompt format and remove the target part
        # But that's complex. Instead, let's manually construct the correct format
